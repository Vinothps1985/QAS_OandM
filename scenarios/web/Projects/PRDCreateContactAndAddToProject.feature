...
@author:Anusha MG
@description:Verify whether user is able to create a new Contact and add contact into Project and delete it.
@project @contact @positive @smoke
@dataFile:resources/testdata/Projects/Create contact and add to project.csv

Scenario: Create contact and add to project and delete contact
   ...
   And take a screenshot
   And store the current url in "contactURL"
   ...
   And wait until "contacts.details.name" to be visible
   And wait until "contacts.details.name" to be enable
   And wait until "contacts.deleteContact.delete.button" to be visible
   And click on "contacts.deleteContact.delete.button"
   And wait until "contacts.deleteContact.popup.title.text" to be visible
   Then assert "contacts.deleteContact.popup.title.text" text is "Delete Contact"
   And wait until "contacts.deleteContact.popup.delete.button" to be visible
   And click on "contacts.deleteContact.popup.delete.button"
   And wait until "contacts.deleteContact.delete.text" to be visible
   Then assert "contacts.deleteContact.delete.text" text is "Contact \"${fullName}\" was deleted. Undo"
   And take a screenshot

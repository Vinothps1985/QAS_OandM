...
Scenario: Create Service Contract with Active Status
   ...
   And store the current url in "serviceContractURL"
   Then scroll until "serviceContract.quickLinks.recurringBillings.viewAll.label" is visible
   And wait until "serviceContract.quickLinks.recurringBillings.viewAll.label" to be present
   And wait until "serviceContract.quickLinks.recurringBillings.viewAll.label" to be enable
   And click on "serviceContract.quickLinks.recurringBillings.viewAll.label"
   And wait until "serviceContract.recurringBillings.rowNumber.text" to be visible
   And take a screenshot
   And wait until "serviceContract.recurringBillings.showMoreActions.text" to be enable
   And click on "serviceContract.recurringBillings.showMoreActions.text"
   And wait until "serviceContract.recurringBillings.showMoreActions.delete.link" to be present
   And wait until "serviceContract.recurringBillings.showMoreActions.delete.link" to be enable
   And click on "serviceContract.recurringBillings.showMoreActions.delete.link"
   And wait until "serviceContract.recurringBillings.showMoreActions.delete.button" to be visible
   And wait until "serviceContract.recurringBillings.showMoreActions.delete.button" to be enable
   And click on "serviceContract.recurringBillings.showMoreActions.delete.button"
   And wait until "serviceContract.recurringBillings.showMoreActions.deleteItems.text" to be present
   And wait until "serviceContract.recurringBillings.showMoreActions.deleteItems.text" to be visible
   Then assert "serviceContract.recurringBillings.showMoreActions.deleteItems.text" text is "No items to display."
   And take a screenshot
   When get "${serviceContractURL}"
   And wait for the page to finish loading
   Then scroll until "serviceContract.quickLinks.recurringBillings.link" is visible
   Then scroll until "serviceContract.quickLinks.recurringBillings.icon.image" is visible
   And wait until "serviceContract.quickLinks.recurringBillings.link" to be present
   And wait until "serviceContract.quickLinks.recurringBillings.link" to be enable
   Then assert "serviceContract.quickLinks.recurringBillings.count.text" text is "(0)"
   And take a screenshot
